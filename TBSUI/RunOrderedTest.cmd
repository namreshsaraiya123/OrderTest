...
:Usage
@echo usage: RunOrderedTest.cmd ORDEREDTESTFILE
@echo Runs an automated regression test given an ordered test file.
goto End
...
@echo Running MSTest...
cd "%SOLUTIONDIR%"
"%MSTEST%" /nologo /testcontainer:"%TESTFILE%" /testsettings:"%PROJECTDIR%\Local.testsettings" /resultsfile:"%RESULTSDIR%\%Datetime%_TestResult.trx" /detail:stdout

@echo Running MSXSL...
@echo with project "%ProjectDir%\%VSProjectName%.csproj"
"%MSXSL%" "%ResultsDir%\%Datetime%_TestResult.trx" "%ProjectDir%\TestResult.xsl" -o "%ResultsDir%\%Datetime%_TestResult.html"
cd "%PROJECTDIR%"
if errorlevel 1 goto ErrMsXsl
...
:ErrMsXsl
@echo Error: msxsl was not found. %MSXSL%
exit /b 1
goto END
